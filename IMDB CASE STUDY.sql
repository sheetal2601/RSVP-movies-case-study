...
+---------------+-------------------+ */
-- Type your code below:
-- Part 1 -- Number of movies released each Year

SELECT year,COUNT(title) AS NUMBER_OF_MOVIES
  FROM movie
 GROUP BY year ;

-- part 2 Number of movies released each month 
...
-- Grouping rows based on movie id
...
-- Action genre has the highest duration of 113 seconds followed by romance and crime genres.
...
-- Finding the number of movies based on median rating and ordering it based on movie count.
...
AS (SELECT production_company,Count(movie_id)
AS MOVIE_COUNT,
RANK()
OVER(
...
-- Answer - yes germen movies get more votes than Italian movies
...
WITH actor_summary AS (
  SELECT
    ...
    COUNT(m.id) AS movie_count,
    ROUND(SUM(rt.avg_rating * rt.total_votes), 2) AS actor_avg_rating,
    RANK() OVER (ORDER BY ROUND(SUM(rt.avg_rating * rt.total_votes) / SUM(rt.total_votes), 2) DESC) AS actor_rank
  FROM
    movie m
    INNER JOIN ratings rt ON m.id = rt.movie_id
...
WITH actress_summary AS (
  SELECT
    ...
    SUM(rt.total_votes) AS total_votes,
    COUNT(m.id) AS movie_count,
    ROUND(SUM(rt.avg_rating * rt.total_votes), 2) AS actress_avg_rating,
    RANK() OVER (ORDER BY ROUND(SUM(rt.avg_rating * rt.total_votes) / SUM(rt.total_votes), 2) DESC) AS actress_rank
  FROM
    movie m
    INNER JOIN ratings rt ON m.id = rt.movie_id
    INNER JOIN role_mapping rm ON rt.movie_id = rm.movie_id
    INNER JOIN names n ON rm.name_id = n.id
  WHERE
    rm.category = 'actress' AND m.country LIKE '%India%'
  GROUP BY
    n.name
  HAVING
    COUNT(m.id) >= 5
...
FROM
  (
    SELECT
      *,
      MAX(NAME) OVER (PARTITION BY name_id) AS director_name
    FROM
